...
class TestImageProcessor:
    ...
    def test_initialization_default(self):
        ...
        assert processor.min_width == 0
        assert processor.min_height == 0

    def test_initialization_with_params(self):
        """Test ImageProcessor initialization with parameters."""
        processor = ImageProcessor(mask_crop=True, min_width=100, min_height=50)
    ...
    def test_crop_from_image_no_mask(
        self, processor_no_mask, sample_image, sample_coords
    ):
        """Test cropping without mask returns PIL Image."""
        result = processor_no_mask.crop_from_image(sample_image, sample_coords)

        assert result is not None
        assert isinstance(result, Image.Image)
        assert result.size == (60, 60)  # 80-20 = 60

    def test_crop_from_image_with_mask(
        self, processor_with_mask, sample_image, sample_triangle_coords
    ):
        """Test cropping with mask (white background) returns PIL Image."""
        result = processor_with_mask.crop_from_image(
            sample_image, sample_triangle_coords
        )
    ...
    def test_crop_from_image_with_valid_size(
        ...
        result = processor_with_filters.crop_from_image(sample_image, large_coords)

        assert result is not None
        assert isinstance(result, Image.Image)

    def test_crop_from_image_invalid_coords(self, processor_no_mask, sample_image):
        """Test cropping with invalid coordinates."""
        # Coordinates outside image bounds
        invalid_coords = [(200, 200), (300, 200), (300, 300), (200, 300)]

        result = processor_no_mask.crop_from_image(sample_image, invalid_coords)

        # Should return None for invalid coords
        assert result is None

    def test_crop_from_image_empty_coords(self, processor_no_mask, sample_image):
        """Test cropping with empty coordinates."""
        result = processor_no_mask.crop_from_image(sample_image, [])
        assert result is None

    def test_crop_from_image_polygon(self, processor_with_mask, sample_image):
        """Test cropping with complex polygon coordinates."""
        # Pentagon shape
        pentagon_coords = [(50, 20), (80, 40), (70, 70), (30, 70), (20, 40)]

        result = processor_with_mask.crop_from_image(sample_image, pentagon_coords)
